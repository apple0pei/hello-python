...
test_file = glob.glob(os.path.join(directory_test[0],"*.jpg"))[1101]
# print test_file
...
X=np.genfromtxt("TutorialTest_X.csv",delimiter=',')
...
print clf;
scores = cross_validation.cross_val_score(clf, X_train, y_train, cv=5, n_jobs=1);
...
#%%
submit_y = []
total_i = len(test_y)-1
for i in range (0,total_i):
    readin_file = glob.glob(os.path.join(directory_test[0],"*.jpg"))[i]
    readin_filename = readin_file.split("/")[8]
    submit_y.append(readin_filename)
#%%
